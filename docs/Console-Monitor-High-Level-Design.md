...
flowchart TB
  subgraph ControlPlane["Control Plane"]
    dce["console-monitor-dce.service"]
    configdb[("CONFIG_DB")]
  end

  subgraph DataPlane["Data Plane"]
    subgraph Link1["Link 1"]
      bridge1["pty-bridge@1.service"]
      proxy1["proxy@1.service"]
    end
    subgraph Link2["Link 2"]
      bridge2["pty-bridge@2.service"]
      proxy2["proxy@2.service"]
    end
    subgraph LinkN["Link N"]
      bridgeN["pty-bridge@N.service"]
      proxyN["proxy@N.service"]
    end
  end

  dce -- monitor --> configdb
  dce -- start/stop --> bridge1
  dce -- start/stop --> proxy1
  dce -- start/stop --> bridge2
  dce -- start/stop --> proxy2
  dce -- start/stop --> bridgeN
  dce -- start/stop --> proxyN
```
